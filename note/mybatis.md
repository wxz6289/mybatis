...
避免几乎所有JDBC模版代码和手动配置参数以及结果集获取 
支持xml配置、注解配置、Java API配置
面向接口编程 半自动ORM框架
...
## 9. MyBatis、Hibernate、JPA 与 JDBC 对比

先理清**四个名字分别是什么**：

- **JDBC（Java Database Connectivity）**：Java 访问数据库的**标准 API**（`java.sql.*`），属于 JDK 规范的一部分。负责连接、执行 SQL、处理结果集等**最底层能力**，不包含 ORM 语义。
- **JPA（Jakarta Persistence API）**：Java **持久化规范**（原 JSR-338），定义实体、`EntityManager`、JPQL、缓存等**抽象接口**。本身不是可运行的 Jar，需要**具体实现**。
- **Hibernate**：最流行的 **JPA 实现**之一，同时也是功能完整的 **ORM 框架**（自带 Session、Criteria、HQL 等，可不只用 JPA 注解）。
- **MyBatis**：**SQL 映射框架**（常称半自动 ORM）。核心是 **Mapper + XML/注解写 SQL**，把结果映射到 Java 对象；**不是 JPA 实现**，编程模型与“面向实体 + 自动生成 SQL”的 JPA/Hibernate 不同。

关系可简记为：

- **JDBC** 几乎有所持久层框架最终都会用到 JDBC（或驱动提供的等价能力）。
- **JPA = 规范；Hibernate = 一种实现**（还可换 EclipseLink 等）。
- **MyBatis 与 JPA 并列选型**：前者偏 SQL 可控，后者偏对象与标准持久化 API。

### 9.1 对比总表

| 维度 | JDBC | MyBatis | Hibernate（作为 ORM） | JPA（规范） |
| --- | --- | --- | --- | --- |
| **定位** | 数据库访问 API | SQL 映射与持久化辅助 | 全功能 ORM 框架 | 持久化标准接口 |
| **SQL 由谁写** | 手写 SQL 字符串 | **开发者写 SQL**（XML/注解） | 多数场景 **ORM 生成**（HQL/JPQL/Criteria） | 依赖实现；常用 JPQL，底层仍由实现生成 SQL |
| **对象映射** | 手动 `ResultSet` → 对象 | 声明式映射（`resultMap` / 别名） | 全自动实体 ↔ 表 | 注解/XML 描述实体，由实现完成映射 |
| **学习曲线** | 低概念量，但样板代码多 | 中等；需会 SQL + Mapper | 较高；需理解持久化上下文、懒加载等 | 中等偏高；先学规范再对接实现 |
| **复杂 SQL / 报表** | 完全可控 | **强项**：动态 SQL、存储过程、优化 SQL | 可做但复杂查询有时别扭（原生 SQL 补救） | 同左，取决于实现 |
| **数据库移植性** | SQL 写在应用里，移植靠自己 | SQL 写在 Mapper，移植需调整 SQL | JPQL/HQL 抽象度高，但原生 SQL 仍绑库 | 实体映射可移植；原生 SQL 仍绑库 |
| **缓存与关联** | 无 | 一级缓存（SqlSession）；二级可选插件级 | 一级/二级缓存、集合抓取策略丰富 | 规范二级缓存接口；实际能力看实现 |
| **典型适用** | 极小工具、框架底层 | 既有 SQL 资产、复杂查询、团队强 SQL | CRUD 为主、领域模型驱动、快速开发 | 标准持久层 API、多实现切换需求 |

### 9.2 JDBC：优点与代价

- **优点**：无额外框架依赖思路简单；任意 SQL 都能写；排障时最接近数据库真实行为。
- **代价**：连接、语句、结果集、事务、异常和资源关闭等**样板代码多**；防注入和参数绑定需自己严守规范；对象映射重复劳动。

### 9.3 MyBatis：优点与代价

- **优点**：**SQL 透明、可审计、易优化**；复杂条件用动态 SQL；与 DBA、遗留 SQL 协作友好。
- **代价**：大量 CRUD 仍需手写或用生成器；对象关联需自己设计查询；团队弱 SQL 时维护成本高。

### 9.4 Hibernate / JPA：优点与代价

- **优点**：实体关系映射成熟；变更表结构时可配合迁移工具；减少重复 CRUD；符合“面向领域模型”的开发方式。
- **代价**：需理解 Session、持久化状态、懒加载、N+1 等问题；复杂 SQL 有时不如手写直观；调试 SQL 需打开 SQL 日志或理解生成逻辑。

### 9.5 选型建议（简要）

- **优先 JDBC**：极小脚本、教学、或你在封装自己的框架底层。
- **优先 MyBatis**：报表与统计多、SQL 复杂、需要精细优化或与遗留数据库深度适配。
- **优先 Hibernate / Spring Data JPA**：领域模型清晰、以 CRUD 与关联为主、希望遵循 JPA 标准与生态。
- **混合**：同一项目里也可局部使用（例如主业务 JPA，报表模块 MyBatis），注意事务边界与数据源配置一致。

### 9.6 小结

- **JDBC** 是地基；**JPA** 是规范；**Hibernate** 是 JPA 的主力实现之一；**MyBatis** 是另一条以 SQL 为中心的持久化路线，与 JPA **不是替代关系而是选型关系**。
- 选型本质常在 **“SQL 可控与性能可见”**（MyBatis）与 **“对象建模与标准 API”**（JPA/Hibernate）之间权衡。